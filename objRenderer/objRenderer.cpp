// objRenderer
// objRenderer uses its objReader function to read and parse a single 3D object's description from a Wavefront .obj file, for subsequent rendering using DirectX.
//
// Author: This program, except for its objReader function, is primarily based on http://DirectXTutorial.com's "DirectX 11 Win32 Desktop: Direct3D: Moving to 3D: Lesson 3: Simple Modeling" and earlier lessons, by Chris Hanson.
//		   The objReader function was written by Robert John Tortorelli.
...
// DirectX Declarations: End

// Global Function Declarations: Function prototypes for internal functions defined in this program.
// The corresponding function definitions in this program are prefaced using a comment of the form:
				   ...
				   int nCmdShow)							// Indicates if the main program window will be minimized, maximized, or shown normally.
{
	...
	// Clean3D function cleans up Direct3D and COM.
	CleanD3D();

	// The program quits by returning control to the operating system.
...
void InitGraphics(void)
{
	...
	// Assign values to the buffer resource description D3D11_BUFFER_DESC structure's members. Any subordinate members (variable.member.subordinatemember) are described in the comments.
	bd.ByteWidth = sizeof(VERTEX) * Vn;						// Assigned a value specifying the size of the buffer in bytes. The vertex buffer resource's size is the size of the VERTEX structure * the number of vertices.
...
void RenderFrame(void)
{
	...
	//      Point, line, triangle and many other types can be specified.
	// ***
